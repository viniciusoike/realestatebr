...
- [ ] Keep `date_start`, `date_end`, and `...` accepted by `get_dataset()`.
...
- [ ] Confirm that `sinapi.rds` and `pim_pf_construction.rds` are published
      in the `cache-latest` release.

## Verification

- [ ] Run focused `get_dataset()` and `query_dataset()` tests.
- [ ] Run the complete test suite.
- [ ] Regenerate documentation with `devtools::document()`.
- [ ] Run `pkgdown::check_pkgdown()`.
- [ ] Run `devtools::check()`.
- [ ] Run `git diff --check` and review the generated NEWS and reference pages.
