...
### Function Naming Hierarchy (3 tiers)

Every dataset file organises its functions into three tiers:

| Tier | Prefix | Role |
|------|--------|------|
| 1 | `get_*` | Orchestrator: validate inputs, route to cache or fresh download, attach metadata, return |
| 2 | `download_*` | Network I/O: download files, scrape pages, call APIs. Returns a file path or a raw tibble. No data cleaning. |
| 3 | `clean_*` | Transform: rename columns, parse types and dates, reshape. Returns a clean tibble. No network calls. |

Structural rules:
- `get_*` never does network I/O — it delegates entirely to `download_*`
- `download_*` never cleans data — it only retrieves and returns raw material
- `clean_*` never makes network calls — it only transforms what it receives
- When a `clean_*` function also involves substantial reshaping or pivoting, split into `clean_*` (rename + type conversion) + `format_*` (pivoting, restructuring). Use this split only when the combined function becomes hard to follow.

```r
# Canonical structure for a dataset file
get_secovi()
  └── download_secovi()    # network I/O, returns raw list/tibble/path
  └── clean_secovi()       # transformation, returns clean tibble
```

### Distinguishing dataset-level `download_*` from helper-level `download_*`

Both the dataset functions and the shared infrastructure use the `download_*` prefix.
...
Dataset-specific functions should always delegate to the generic helpers rather than calling
`utils::download.file()` or `rvest::read_html()` directly.

**Legacy note:** `import_bcb_realestate()`, `import_secovi()`, `import_secovi_robust()`, and
`import_bcb_series_robust()` use the old `import_*` naming but serve the download role. Rename
these to `download_*` in future refactors.

### Utility Function Prefixes

| Prefix | Role | Examples |
|--------|------|---------|
| `parse_*` | Narrow value-level conversion | `parse_cgi_number()`, `parse_bis_detailed_columns()` |
| `build_*` | Construct helper objects (column vectors, lookup tables) | `build_fipezap_col_names()` |
| `validate_*` | Data quality checks | `validate_abecip_data()`, `validate_dataset()` |
| `calculate_*` | Derive new columns from existing data | `calculate_rppi_changes()` |
...
For helpers that serve only one dataset, prefix with the dataset name to make scope obvious:

```r
# Good — dataset prefix clarifies ownership
abrainc_basic_clean(df, subcategories)
secovi_extract_header(x)

# Avoid — generic name obscures that it belongs to one dataset
clean_basic(df)          # only used in get_secovi.R
process_sbpe_data(df)    # only used in get_abecip_indicators.R
```
...
Standard names for frequently used local objects. Pick one and apply it consistently.

| Object | Standard name | Notes |
|--------|--------------|-------|
| Temporary file path | `temp_path` | `tempfile(fileext = ".xlsx")` |
| Column rename mapping | `cols_rename` | Named vector: `c("new_name" = "old_name")` |
| Column selection vector | `cols_select` | Character vector of names to keep |
| URL of page to scrape | `url_page` | The landing page, not the file link |
| Final download URL | `url` | Use `url_download` only when `url_page` and the file URL coexist in the same scope |
| Raw import | `raw` | Direct output of `read_excel()` / `read_csv()` before any mutation |
| Intermediate data frame | `df` | In-progress transformation steps |
| Primary data object | dataset name | Use the short name (`igmi`, `abecip`, `secovi`) for the main series being built |

### On the `.` Prefix for Local Variables

Do not use a `.` prefix for ordinary local variables (`.col_names`, `.temp_path`). In R, `.`
conventionally marks non-data function arguments (`.data`, `.by`) and hidden internal objects.
Applying it to local variables adds noise without conveying meaning.

Exception: module-level constants shared only within a file (e.g. `.rppi_cols` in `get_rppi.R`)
are acceptable — use sparingly.

### Examples

```r
# Column vectors
cols_rename <- c("date" = "mes", "value" = "valor")
cols_select <- c("date", "name_muni", "index", "chg", "acum12m")
...
# When both the page URL and file URL exist in the same function
url_page  <- "https://www.abecip.org.br/indicadores"
url       <- rvest::html_attr(rvest::html_element(parsed, xpath = xpath), "href")
temp_path <- download_excel(url)
```
