...
**Branch**: `dev/v0.6.0` (to be created)
...
1. **Strip Examples from Internal Functions** (38 @examples blocks)
   - Target files with most examples:
     - get_cbic.R (17 examples)
     - cache.R (1 example)
     - cache-user.R (3 examples)
     - cache-github.R (2 examples)
     - get_*.R files (remaining ~15 examples)

   **Keep**: @title, @description, @param, @return, @source, @references, @keywords internal
   **Remove**: @examples, verbose @details, @section blocks

2. **Consolidate Cache Documentation**
   - Create unified documentation family for cache functions
   - Simplify deprecated cache.R warnings
   - Cross-reference user cache vs GitHub cache vs deprecated functions

3. **Success Metrics**
   - Reduce average internal function doc from ~50 lines to ~15 lines
   - Remove ~50-75 example blocks
   - Maintain all parameter and return value documentation
