...
#' International Settlements (BIS) with modern error handling, progress
...
get_bis_rppi <- function(
    ...
    tryCatch(
      {
        ...
        if (!quiet) {
          ...
          cli::cli_inform(
            "Successfully loaded {total_records} BIS RPPI records from cache"
          )
        }
  ...
  if (!quiet) {
    total_records <- if (is.list(df)) sum(sapply(df, nrow)) else nrow(df)
    cli::cli_inform("Successfully processed BIS RPPI data with {total_records} total records")
  }
...
#' Get BIS RPPI Selected Data with Robust Error Handling
#'
...
#' @keywords internal
get_bis_rppi_selected_robust <- function(quiet, max_retries) {
  if (!quiet) {
    cli::cli_inform("Downloading BIS selected RPPI Excel file...")
  }
  ...
  if (!quiet) {
    cli::cli_inform("Processing BIS selected RPPI data...")
  }

  return(process_bis_selected_data(temp_path, quiet))
}

#' Get BIS RPPI Detailed Data with Robust Error Handling
#'
...
#' @keywords internal
get_bis_rppi_detailed_robust <- function(quiet, max_retries) {
  if (!quiet) {
    cli::cli_inform("Downloading BIS detailed RPPI Excel file...")
  }
  ...
  if (!quiet) {
    cli::cli_inform("Processing BIS detailed RPPI data...")
  }
...
process_bis_selected_data <- function(temp_path, quiet) {
  ...
  if (!quiet) {
    cli::cli_inform("Cleaning and merging BIS selected data...")
  }
...
process_bis_detailed_data <- function(temp_path, quiet) {
  ...
  if (!quiet) {
    cli::cli_inform("Processing {length(sheets)} detailed data sheets...")
  }
