...
itbi_cols <- list(
  names = c(
    ...
    "house_zipcode",
    "itbi_transaction",
    "itbi_transaction_value",
    "itbi_transaction_date",
    "itbi_valor_venal",
    "itbi_proportion",
    "itbi_valor_venal_ref",
    "itbi_base_value",
    "house_financing_method",
    "house_financed_value",
    "house_registry",
    "house_id_matricula",
    "iptu_sql_status",
    "house_land_area",
...
clean_itbi <- function(data) {
  ...
  clean_df <- clean_df |>
    dplyr::mutate(
      ...
      house_address = stringr::str_glue(
        "{house_street}, {house_number}, {house_neighborhood}, S\u00e3o Paulo"
      ),
      house_address = stringr::str_replace_all(
        house_address,
        pattern = "  ",
        replacement = " "
      )
