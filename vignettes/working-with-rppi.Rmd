...
This vignette provides a brief introduction to Residential Property Price
Indices (RPPI) in the `realestatebr` package. Since `realestatebr` returns
tibbles by default, we recommend using it together with `dplyr`.
...
Before getting into the details, it is useful to identify which indices are
best suited to different applications.

### The best indices

The IGMI-R is a useful choice for sale prices, while the IVAR is a useful choice
for rent prices. FipeZap is the main option for commercial prices. These indices
use different source data and methodologies, so the best choice depends on the
question being studied.

The IGMI-R is a hedonic price index based on transaction prices from housing
mortgages. Its main limitations are its relatively short time span
(2014-present) and geographic coverage (major cities only).

The IVAR is a repeat-rent index based on residential rental contracts. The
dataset has a relatively short time span (2019-present) and limited geographic
coverage (four cities).

FipeZap has the broadest geographic (50+ cities) and temporal coverage
(2008-present), but is based on median listing prices rather than transaction
prices. It is therefore best suited to questions about advertised prices, not
transaction prices.

The IGMI-R and IVAR are practical choices for sales and rent comparisons,
respectively, when their coverage and methodologies fit the application.
...
For convenience, the stacked version of the dataset implicitly only includes
`rooms == "total"` for FipeZap. Also, "Brazil" is listed under `name_muni` for
the nationwide index, rather than a city-specific name.

The plot shows the three sale indices at a national level. While there is some
convergence between the indices, there is a growing gap between them in the
post-pandemic years. This highlights the importance of choosing an index whose
coverage and methodology fit the use case.
...
The plot emphasizes the main strength of FipeZap's index: its broad geographic
coverage. The example uses cities with observations starting in 2021.
...
Each index has its own base period, so normalization is required before
