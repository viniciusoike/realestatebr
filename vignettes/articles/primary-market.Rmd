...
This article tracks the development cycle, from launches to sales and deliveries, and the business environment around it using three datasets.
...
As with other articles and vignettes, the goal is not to provide a deep-dive
analysis but rather to explore the datasets in `realestatebr`. Besides the
usual `dplyr` and `ggplot2`, we will also use the `trendseries` package to
quickly estimate trends and cycles.
...
FGV has a rich series of surveys and indicators, and `realestatebr` collects
the most relevant ones for the construction and real estate sector. This
dataset contains 15 series identified by the `name_series`/`code_series`
columns.
...
It's important to note that this historical indicator has been discontinued.
I'm considering replacing it with my own version or recreating it based on the
publicly available methodology. For more updates, see the [GitHub issue](https://github.com/viniciusoike/realestatebr/issues/6).

## São Paulo

São Paulo is Brazil's largest real estate market. The `secovi` dataset covers
São Paulo and the São Paulo Metropolitan Region. The ABRAINC dataset includes
a real estate leading indicator based on building permits in São Paulo.
...
SECOVI publishes data for São Paulo and the São Paulo Metropolitan Region. The
`launch` and `sale` tables track new supply and sales in the city.
...
Some tables provide more detail, such as `sales`. The plot below shows sales by
the number of rooms.

```{r}
sp_market_rooms <- secovi |>
  filter(grepl("sales_[0-9]rooms", variable)) |>
