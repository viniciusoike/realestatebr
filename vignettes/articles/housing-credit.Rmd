...
(SBPE) and by the FGTS workers' fund. There are three main datasets that hold
housing credit information, in increasing order of complexity and detail:
`abecip`, `bcb_series`, and `bcb_realestate`.
...
It's important to note that `realestatebr` follows its source datasets closely.
As such, monetary values are reported in nominal Brazilian reais and are not
adjusted for inflation. This disclaimer applies to all three datasets presented
in this article.
...
```{r}
library(ekioplot)
color_palette <- ekio_pal()
theme_series <- function() {
  theme_ekio(base_size = 10) +
    theme(
...
We now use the `bcb_realestate` dataset to get more details about the rest of
the funding market. This is quite a complex dataset to manipulate, and
discovering how it works can take some time. Luckily, the code below is quite
simple and gets the total amount of credit funding from private sources for real
estate loans in Brazil.
...
gathers the most relevant real estate credit series. Note that while we use the
`name_simplified` column to filter series, the `bcb_series` dataset includes a
`code_bcb` column that matches the original codes from SGS.
...
For a more detailed view of credit rates, we use the `bcb_realestate` dataset.
The code below shows how to find the rates for all credit lines at the national
(`BR`) level. It's also important to note that `pf` is short for `pessoa
física` (i.e. private individual), while `pj` is short for `pessoa jurídica`
(i.e. private corporation).
...
help topics `?abecip`, `?bcb_series`, and `?bcb_realestate`. For price indices,
see the vignette `vignette("working-with-rppi")`.
