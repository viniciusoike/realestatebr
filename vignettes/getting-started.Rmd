...
The package now provides a modern, unified interface for accessing all datasets with automatic fallback strategies and consistent data formatting.

```{r unified, eval=FALSE}
# Discover available datasets
datasets <- list_datasets()
head(datasets[, c("name", "source", "description")])

# Get detailed dataset information
info <- get_dataset_info("abecip")
str(info$categories, max.level = 2)

# Access data with automatic fallback (GitHub cache → fresh download)
...
All existing `get_*()` functions continue to work unchanged:
...
Abecip is one of the backbones of the Brazilian real estate market. The data is available in two main tables: "SBPE" and "Financiamentos". The example below shows how to download both tables and plot the total amount of financing for new properties.

```{r}
abecip <- get_dataset("abecip", source = "github")
```

```{r}
sbpe <- abecip$sbpe |>
  filter(date >= as.Date("2010-01-01")) |>
  mutate(acum = RcppRoll::roll_sumr(sbpe_netflow, n = 12, fill = NA)) |>
  filter(!is.na(acum))

ggplot(sbpe, aes(date, acum)) +
  geom_line() +
  geom_hline(yintercept = 0) +
  scale_x_date(date_breaks = "2 year", date_labels = "%Y") +
  scale_y_continuous(labels = scales::label_number(scale = 1e-3, suffix = "B")) +
  labs(
    title = "Total SBPE Net Flow (12-month rolling sum)",
    y = "R$ (Billion)",
    caption = "Source: Abecip"
  )
```
