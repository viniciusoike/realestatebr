...
To get multiple indexes use the `get_rppi()` function with `stack = TRUE`.

```{r}
sales_index <- get_rppi(category = "sale", stack = TRUE)

ggplot(sales_index, aes(x = date, y = acum12m, color = source)) +
  geom_line()
```

The FipeZap Index has the most 
