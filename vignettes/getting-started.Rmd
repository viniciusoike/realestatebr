...
This vignette provides a minimal introduction to the `realestatebr` package,
showing how to use its core functions. Since `realestatebr` returns tibbles by
default, we recommend using it together with `dplyr`.
...
- Use `get_dataset()` to retrieve datasets.
...
mechanism for residential mortgages in Brazil. The table `sbpe` from `abecip` tracks the deposits and withdrawals from savings accounts, which help finance real estate construction and acquisition.
...
The `bcb_realestate` dataset provides a broad collection of real estate
statistics from the Brazilian Central Bank. This is a relatively large dataset
and exploring it can be cumbersome. Each series is uniquely identified by
`date` and `series_info`. Helper columns `v1`, `v2`, ..., `v5`, `abbrev_state`,
`category`, and `type` simplify the use of the dataset.
...
As a final warning, note that the `bcb_realestate` dataset uses end-of-month
dates, such as `2023-01-31`, while many other datasets use the first day of the
month. This can cause issues when merging datasets.
